...
class VLNavigationBarView: UIView {
    ...
    init( theSuperView: UIView) {
        ...
        let frame = CGRect(x: 0.0, y: 0.0, width: theSuperView.frame.width, height: VLConst.kNavigationHeight)
    ...
    private func setupUI() {
        ...
        self.statusBar = UIView(frame: CGRect(x: 0.0, y: 0.0, width: self.bounds.width, height: VLConst.kStatusBarHeight))
        self.addSubview(self.statusBar)
        
        self.navigationBar = UIView(frame: CGRect(x: 0.0, y: VLConst.kStatusBarHeight, width: self.bounds.width, height: VLConst.kNavigationBarHeight))
        self .addSubview(self.navigationBar)
        
        self.leftBarButtonView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: self.bounds.width/3.0, height: VLConst.kNavigationBarHeight))
        self.navigationBar.addSubview(self.leftBarButtonView)
        
        self.rightBarButtonView = UIView(frame: CGRect(x: self.bounds.width*2.0/3.0, y: 0.0, width: self.bounds.width/3.0, height: VLConst.kNavigationBarHeight))
        self.navigationBar.addSubview(self.rightBarButtonView)
        
        self.contentTitleView = UIView(frame: CGRect(x: 0.0, y: 0.0, width: self.bounds.width/3.0, height: VLConst.kNavigationBarHeight))
        contentTitleView.center = CGPoint(x: self.bounds.width/2.0, y: VLConst.kNavigationBarHeight/2.0)
        self.navigationBar.addSubview(self.contentTitleView)
        
        self.lineView = UIView(frame: CGRect(x: 0.0, y: VLConst.kNavigationHeight, width: self.bounds.width, height: 1.0))
        lineView.backgroundColor = VLConst.kHexColor(0xE9EAEA)
...
// MARK: - 使用到的全局变量
struct VLConst {
    static let kStatusBarHeight: CGFloat = VLConst.kIsIPhoneX() ? 44.0 : 20.0
    static let kNavigationHeight: CGFloat = VLConst.kStatusBarHeight + kNavigationBarHeight
    static let kNavigationBarHeight: CGFloat = 44.0
    //判断手机是否是iPhoneX
    static func kIsIPhoneX() -> Bool {
    ...
    static func kHexColor(_ rgbValue: Int) -> UIColor {
